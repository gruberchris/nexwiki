...
| **Eval Min Train Cases** | — | `NEXWIKI_EVAL_MIN_TRAIN` | `30` | Minimum train cases the evolution eval gate accepts. Smaller uploads are refused with per-issue errors; a set-but-unparseable value fails closed. |
| **Eval Min Val Cases** | — | `NEXWIKI_EVAL_MIN_VAL` | `10` | Minimum val cases the evolution eval gate accepts. Same fail-closed behavior as the train threshold. |
| **Job Retrieval Top-K** | — | `NEXWIKI_JOB_RETRIEVAL_TOPK` | `5` | For evolution jobs configured with `injection_mode: retrieve` (story 11): how many top-ranked skills (BM25 over the registry's name/description front matter vs the task keywords) the runner quotes into the stdin payload, always plus the skill under evolution. A coarse server-side pre-filter; full injection remains the default. A set-but-unparseable value fails closed at startup and at every step. |

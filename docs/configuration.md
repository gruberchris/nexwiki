...
| **Network Bind Host** | `-bind` | `NEXWIKI_BIND` | `127.0.0.1` (native) / `0.0.0.0` (Docker) | Network interface to bind. Defaults to `127.0.0.1` for local desktop security. Inside containers, defaults to `0.0.0.0` to permit container port publishing. Set to `0.0.0.0` (or `::` for IPv6) to listen on all interfaces, or a specific address such as a LAN IP or `::1`. An `-mcp-only` process uses this host to locate a web server bound to a specific address. |
| **Agent Attribution Fallback** | `-agent-name` | `NEXWIKI_AGENT_NAME` | `(unset)` | Fallback attribution identity recorded in the activity log for MCP clients that do not report a name. An `-mcp-only` sidecar forwards the stdio client's reported name to the web server in the `X-NexWiki-Client-Name` header; attribution prefers per-request `clientInfo`, then the transport's identity (the stdio handshake or the forwarded header), then this fallback, then `AI Agent`. *(Note: This is distinct from `-name`, which is the wiki title).* |
| **Allowed Browser Origins** | — | `NEXWIKI_ALLOWED_ORIGINS` | Loopback only | Comma-separated list of browser origins permitted to communicate with the API (e.g., `https://wiki.example.com`). Required when serving NexWiki from a custom DNS domain name: the hostname of each listed origin is also an accepted `Host` header value. |
| **Archive Auto-Delete** | — | `NEXWIKI_AUTO_DELETE_ARCHIVED_AFTER_DAYS` | `0` (disabled) | Number of days after an article is archived before it is permanently deleted during server startup sweeps. Documents other documents still link to are kept (as are ones a scan that skipped entries cannot show unlinked); plans are left to the plan lifecycle worker, and a failed delete is logged without affecting startup. |
...
NexWiki also validates every request's `Host` header, before origin checks and handlers run. Accepted Host values: IP literals (IPv4 or IPv6), `localhost` and `*.localhost`, the configured `-bind` hostname, and the hostname of every origin listed in `NEXWIKI_ALLOWED_ORIGINS` (setting `NEXWIKI_ALLOWED_ORIGINS="*"` accepts any Host). Any other Host is refused with `403 Forbidden` and a hint pointing at `NEXWIKI_ALLOWED_ORIGINS`.

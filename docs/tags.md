...
*Note: NexWiki does not allow global tag renaming. To rename a tag, simply apply the new tag name to the desired articles and delete the old tag.*
...
* **Explicit Search Bypassing**: If you explicitly search using the `aiagent-` tag name (e.g. searching `tags:aiagent-plan` or `aiagent-plan`), the exclusion is bypassed, and the matching agent memories are successfully returned.
...
To see all your frontend guides, simply click the `frontend` tag pill in your sidebar tag cloud.

### 2. AI-Driven Plan Tracking (`aiagent-plan`)
...
```
This is saved as an article with the tag `aiagent-plan`. As the agent completes milestones, it calls `append_agent_memory` to log accomplishments. The page remains safely stored under your **🤖 AI memories** directory, keeping your main wiki page list clean.

### 3. AI-Driven Troubleshooting Log (`aiagent-troubleshooting`)
If a server build fails, the agent can document the investigation:
* **Title**: `Troubleshooting Go Build Error May 2026`
* **Tags (Auto-applied)**: `aiagent-troubleshooting`, `aiagent-troubleshooting-backend`
...
* **Benefit**: The next time a build error occurs, the agent (or you!) can explicitly search for `aiagent-troubleshooting` to find past resolutions instantly, avoiding repeated debugging.

...
* **MDLINK_BROKEN**: Warns on absolute internal Markdown links — `[text](/articles/slug)` — whose target slug does not exist. This is the form most internal links in a real wiki take, so leaving it unchecked meant the majority of internal links got no warning at all. Image links (`![alt](/articles/…)`) and `/api/articles/…` URLs are not flagged.

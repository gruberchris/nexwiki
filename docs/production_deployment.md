...
* **Full Access Granted**: Any client or browser that can route network traffic to NexWiki's port has unrestricted permissions:
  ...
  * Execute all 9 Model Context Protocol (MCP) tools (including `delete_article`).
...
* **Backup Strategy**: Backing up NexWiki is as simple as creating a snapshot or tar archive of the data directory. Alternatively, use the built-in OKF bundle export endpoint (`GET /api/okf/export`, also the web UI's OKF export) to create a clean, portable `.zip` backup at any time.

### Article Files on Disk:
* **Atomic Saves**: Article saves, version-history snapshots, and asset uploads are written to a temporary `.nexwiki-<n>.tmp` file in the same directory, synced to disk (fsync), and renamed into place, so anything reading an article, snapshot, or asset sees the old file or the new one, never a half-written one. Leftovers from a crash are removed at startup: from `articles/` before the home page is seeded, and from `history/` and `assets/` in the background.
* **Sync Clients**: The temp files are dotfiles, but Syncthing, Dropbox, and similar tools sync dotfiles by default and may briefly pick one up before the rename. Exclude `.nexwiki-*.tmp` if your client supports ignore patterns.
* **Replaced, Not Rewritten**: Because a save renames a new file over the old one, hard links to an article file keep the old content, the saved file is owned by the user NexWiki runs as, and on Unix a read-only article file is replaced rather than rejected (the rename needs write permission only on the directory). The file's permission mode is kept. If that needs a `chmod` and the mount rejects it (some SMB/CIFS and FUSE mounts), the save still succeeds with the new file's default mode and a warning is logged.
* **Unreadable Files**: An article file NexWiki cannot read or parse (for example, front matter broken by an external edit), or an article folder it cannot list, is left out of listings and link scans, but never silently. The server logs `Warning: skipping unreadable article file <path>: <error>` (for a folder, `Warning: skipping unreadable article directory <path>/: <error>`) to stderr (`journalctl -u nexwiki`, `docker compose logs nexwiki`), with `<path>` relative to `articles/`. Each is logged once per file version per server run: again only if the file changes and still fails or breaks again after being fixed, and once more at startup after a restart if it is still broken. The `wiki_health` MCP tool lists the skipped entries with their errors (up to its `limit`) and always reports their full count, as does `get_wiki_overview(include_stats: true)`. If `articles/` itself can be listed but not searched (read permission without execute), scans fail with `article directory is not searchable`, and the server will not start.

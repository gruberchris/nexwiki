...
func TestSubscriptionStreamDeliversLiveUpdates(t *testing.T) {
	...
	// Give the stream a moment to register with the bus, then publish a change.
	...
	// Closing the client side is the cancellation signal on Streamable HTTP.
	_ = req.Body.Close()
	select {
	case <-done:
	case <-time.After(3 * time.Second):
		t.Log("stream still open; reading what it produced so far")
	}

	if ct := rec.Header().Get("Content-Type"); ct != "text/event-stream" {
		t.Errorf("subscriptions/listen must respond with an SSE stream, got %q", ct)
	}
	...
	if len(methods) == 0 {
		t.Fatalf("stream produced no messages:\n%s", rec.Body.String())
	}
	// The acknowledgment MUST come first; no notification may precede it.
	if methods[0] != "notifications/subscriptions/acknowledged" {
		t.Errorf("first message must be the acknowledgment, got %q", methods[0])
	}
	if !containsString(methods, "notifications/resources/updated") {
		t.Errorf("expected a resources/updated for the watched article, got %v", methods)
	}
	if !containsString(methods, "notifications/resources/list_changed") {
		t.Errorf("expected a list_changed for the created article, got %v", methods)
	}
}

...
1. `search_wiki` for the topic. If results are thin, scan the index — `search_wiki` without a query — filtered by `type`, `status`, or `tag`.
2. `read_article` on the most relevant hits; its backlinks list the pages that link to each
   one, so follow them to related decisions, hopping the knowledge graph as needed.

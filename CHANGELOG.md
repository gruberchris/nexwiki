...
- **Enforced plan status lifecycle with a background transition worker.** A plan's status was an advisory convention — thirteen loosely-defined tags shared with every document type, nothing validating them anywhere, with the predictable synonym sprawl (`done`≈`completed`, `wip`/`in-progress`/`active`≈working) and plans carrying zero or two statuses at once. The vocabulary is now closed to **eight states reserved for `AI-Agent-Plan` documents** — `draft`, `implementing`, `blocked`, `completed`, `superseded`, `parked`, `evergreen`, `archived` — with **exactly one per plan enforced at every write path** (editor, REST, MCP, revert, import, global tag deletion) through the single storage choke point they all share. `create_agent_plan` defaults new plans to `draft`; values are validated strictly while unusual *transitions* are deliberately log-and-allow, so a human correcting a mis-tagged plan always wins.
  - **`status_changed_at`** joins the OKF front matter on plans, stamped on every status change. The lifecycle timers run off it — never the article timestamp, so fixing a typo in a completed plan cannot restart its archive clock. The plan's header shows it ("completed since 2 months ago"), making an approaching auto-archive visible rather than a surprise. Absent means *not yet eligible*, never "infinitely old".
  - **A background worker automates the tail of the lifecycle** (web primary only — never under `-mcp-only`, where a sidecar must not mutate a directory the primary owns): `completed`/`superseded` → `archived` after `NEXWIKI_PLAN_ARCHIVE_AFTER_DAYS` (default 90), `archived` → permanently deleted after `NEXWIKI_PLAN_DELETE_AFTER_DAYS` (default 365), sweeping once at startup and then every `NEXWIKI_PLAN_LIFECYCLE_INTERVAL_DAYS`. `parked` and `evergreen` never move — that exemption is their purpose. Because the deletion stage has no human in the loop, it carries three guards: `NEXWIKI_PLAN_LIFECYCLE_DRY_RUN` logs intended transitions without applying them, every transition lands in the durable activity log and over SSE, and a **backlink guard refuses to auto-delete any plan other documents still link to**, reporting the refusal instead.
  - **A one-time startup migration** (marker-gated, never re-runs) remaps legacy statuses onto the closed vocabulary (`wip`/`in-progress`/`active`→`implementing`, `done`→`completed`, `todo`/`ready`→`draft`), collapses multi-status plans to the one that is most true (terminal states win), defaults status-less plans to `draft`, backfills `status_changed_at` to the migration date — **not** the article timestamp, which would put a mature wiki's months-old completed plans on an immediate archive countdown — and strips plan-exclusive statuses from non-plan documents. OKF bundle import and version revert normalize legacy plan tag sets the same way, so old bundles and old revisions stay importable.
  - `get_status_tags` is now type-aware, returning `plan_status_tags` and `general_status_tags` as separate groups (`status_tags` remains the union for compatibility). `wiki_health` reports a per-state plan census and treats `evergreen` like `parked` — a decision, not staleness. Plan statuses cannot be deleted globally.
  - **Frontend**: each lifecycle state gets its own badge palette (not eight grays); the Agent Plans dashboard default becomes the inclusion list `draft || implementing || blocked` (superseding the interim `!completed`); and archived documents are hidden from the dashboard sections and sidebar by default — typing `archived` in a filter reveals them, and direct URLs always work. Hiding is done in the UI layer, not `ListArticles`, so wikilinks to archived pages keep rendering as valid links.
  - Reviving a plan out of `archived` now clears `archived_at`, closing a one-way door: the stamp was written once and never cleared, so an untagged-but-once-archived document stayed hidden from search and on the deletion clock forever.
...
- **⚠️ `draft` and the other plan statuses are no longer valid on wiki articles, memories, or skills** (`archived` stays wiki-wide, with its manual archive semantics unchanged for non-plans). Skills that used the `draft` → `ready` maturity convention should use `wip` → `ready`; the migration strips stray plan statuses from non-plan documents once.

...
- **BREAKING: creating a memory requires `memory_kind`, `description` and `source`**: `save_article` refuses to create an `AI-Agent-Memory` — or to change a document's type into one — unless `memory_kind` is a valid kind and `description` and `source` are non-blank, and the refusal names and explains every missing field. On a type change, the document's existing description and source count when the call omits them. `POST /api/articles` with a memory type and a `PUT /api/articles/{slug}` that changes the type into a memory apply the same rule (400). An ordinary edit of an existing memory is not gated: omitted fields are preserved, and a memory written before the rule stays editable. This restores the write gate `create_agent_memory` enforced since 0.14.0, which was lost when 0.21.0 folded memory creation into `save_article`.
- **The connect-time MCP instructions are short and generic**: they orient an agent without naming any document — call `get_wiki_overview` once at session start and follow its `pinned_memories` (the operator's standing preferences and corrections), search before writing, save plans and memories with `save_article`, and keep session discipline (each orientation call once, a "not found" is an answer, retry a version conflict once with the version the error names).
- **Universal rules moved into the tool descriptions**: `save_article` says that `content` always replaces the whole body (pass the current body back to change only metadata or status), that a `[[WikiLink]]` must name a document in this wiki rather than an agent's local files or tools, and what a new memory requires. `delete_article` says it is irreversible — history, assets and backlinks go with the document — and points at `type` and `purge_history` on `save_article` for the jobs it is misused for.

### Removed
- **BREAKING: the `nexwiki-agent-guidelines` document is retired**: it is no longer seeded at startup, referenced by the connect-time instructions, or exempt from `wiki_health`'s unreferenced-skill check, and no code names a governance document any more. Its universal rules now live in code — tool descriptions, server-side checks, and the connect-time instructions — and operator-specific conventions belong in ordinary `feedback` and `user` memories, which `get_wiki_overview` surfaces as `pinned_memories`. An existing wiki keeps its page as an ordinary skill: move any conventions of your own it holds into `feedback` memories, then delete it.

### Fixed
- **The repeat-lookup damper's strongest notice pointed at retired tools**: the third repeat told the agent to "create it now with the relevant create_* tool" and cited a section of the retired guidelines page. It now points at `save_article`.
- **The `project_planning_workflow` prompt closed a plan with a call that would drop its body**: it said to set `status: "completed"` with `save_article` without saying that `content` replaces the whole body. It now says to pass the current body back unchanged.

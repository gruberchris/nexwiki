...
### Added
- **WikiSkill Evolution Story 13 — The wizard owns the flow**:
  - The evolution wizard is now user-initiated at every step: arriving via "Train with WikiSkill" marks the skill selected, and every step ends with one line of narration plus a Next action that is enabled exactly when the step's work is done — no step leaves the operator waiting on an unseen background job without visible progress and a way to act.
  - New operator-run REST endpoints (thin orchestrations of the existing machinery — no new state machines, no gate changes, MCP tool count unchanged): `POST /api/skills/{slug}/train/start` creates the evolution job server-side (skill lock + one-run-per-skill refusal included) and returns the job view plus the per-harness token ONCE (kept in server memory only, never persisted or logged; the operator's tab may pass it back as `job_token` after a server restart); `POST /api/evolution/jobs/{id}/eval` runs the REAL story-05 format gate on filename+content (per-issue fix-it refusals surfaced verbatim, S0 baseline computed at upload); `POST /api/evolution/jobs/{id}/dispatch` starts the loop server-side through the story-04/07 dispatch path in a background goroutine (sweep first, paused runs resume from their checkpoint, audited like pause/abort/approve, double-dispatch refused with 409).
  - Wizard step 2 (Data) gains guided entry — textarea paste or file upload across JSONL/JSON/CSV/text formats, a client-side format hint, and the gate's per-issue refusals rendered as a fix-it list; step 3 (Baseline) shows the upload-computed S0 with dry-run samples and explains there is nothing to rerun; step 4 (Evolve) gains the "Start the loop" button with the plain-language run contract (headless, ~90 min cap, pause/cancel at any moment) plus the existing live view and controls, and a Resume control for parked runs.
  - Token hygiene: the wizard shows the once-only run token in a copy-able disclosure for operators who prefer their own CLI; nothing secret is stored, and the runner keeps scrubbing the token from captured logs.
- **WikiSkill Evolution Story 12 — Stability S0 pilot harness + runbook (Phase E/E5)**:
  - In-repo pilot demonstration (`server/skill_pilot.go`, run with `go test ./server/ -run TestStabilityPilot -count=1`): seeds the KimmyDB Cluster Stability Protocol skill with 6 train / 4 held-out val scenarios and runs the full evolution loop in-process — the real story-05 eval upload gate, real sandboxed verdict scorer, real jobs, real loop stepper, trained marker, and the story-09 result report — under a deterministic scripted harness shim (no network, no live cluster, no model calls). The E5 success bar is asserted mechanically: false bugs on val at least halved versus the S0 baseline, zero true-detection loss, exactly one result report, and rejected attempts preserved in the audit trail; the run prints a pilot summary with the success verdict.
  - Pilot runbook: the canonical operator procedure for the real run against a live wiki (`server/skill_pilot_runbook.go`, mirrored as a runbook section in `docs/aiagent_skills.md`) — create skill if absent → create job with injection mode → upload eval sets via MCP with the scenario tables inline → dispatch with a real OpenCode/Claude profile → monitor iterations → verify the success bar → human promote/rollback. A test holds the runbook against the actual code paths (registered tool names, env vars, endpoints) and keeps the docs mirror in sync.
...
### Fixed
- The shipped wizard left the operator stuck on Step 1: job creation, eval upload, and dispatch were designed as harness/MCP-side steps the UI merely rendered — the wizard now performs them itself from the browser.

### Changed
- `WizardRunHeader`'s paused and no-run banners now point at the wizard's own Resume control and Data step instead of sending the operator back to the harness.

...
- **Lifecycle status is now a first-class `status` field, with enforced vocabularies for agent plans and agent skills.** Status used to be a tag, and that was the source of a long tail of awkwardness: a status is a single value with a state machine, while tags are an unordered folksonomy, so storing one inside the other forced "exactly one" counting, precedence tables to collapse duplicates, and a denylist to catch an agent writing `wip` when it meant `implementing`. A dedicated field makes the invalid states unrepresentable rather than merely detectable.
  ...
  - **A one-time startup migration** (marker-gated, never re-runs) moves status out of tags into the field: plans remap legacy words (`wip`/`in-progress`/`active` → `implementing`, `done` → `completed`, `todo`/`ready` → `draft`), collapse multi-status cases by terminal-state precedence, default to `draft`, and backfill `status_changed_at` to the migration date — **not** the article timestamp, which would put a mature wiki's months-old completed plans on an immediate archive countdown. Skills remap onto their own three. Wiki articles and memories keep every tag they have except a recognized status word, which moves into the field verbatim with no vocabulary imposed; `archived` stays a tag on those types because there it is a *mechanism* (it stamps `archived_at` and hides the document from search), not a label.
  - **New MCP arguments, no new tools** (the count stays 29): `status` on `create_agent_plan`, `edit_agent_plan`, `create_agent_skill`, `edit_agent_skill`, `create_wiki_article`, and `edit_wiki_article`; a `status` filter on `list_agent_plans`; and `get_status_tags` now returns `plan_status_tags`, `skill_status_tags`, and `general_status_tags` as separate groups (`status_tags` remains their union). REST `POST`/`PUT /api/articles` accept `status` with the same omit-means-preserve semantics.
  - **Frontend**: a Status control sits beside the Tags row in the editor — a dropdown of the class vocabulary for plans and skills, a free-text box for everything else — and status renders as a colored badge on dashboard cards and in the article header. The Agent Plans dashboard default becomes the inclusion list `draft || implementing || blocked`, which keeps working because filters match the status field alongside title and tags. Archived documents are hidden from the dashboard sections and the sidebar by default; typing `archived` in a filter reveals them, and direct URLs always work. Hiding lives in the UI layer, not `ListArticles`, so wikilinks to archived pages keep rendering as valid links.
...
- **⚠️ Lifecycle status moved out of the tag list.** Existing status tags are migrated into the `status` field on first boot; after that, plans and skills reject lifecycle words as tags. Wiki articles and memories are unaffected — their tags are never policed, and the migration leaves everything except a recognized status word exactly where it was. Skills keep the `draft` → `ready` promotion convention, now expressed through the field.

...
NexWiki contains an always-on Model Context Protocol (MCP) server exposing **39 built-in tools** for search, content retrieval, memory management, and agent planning.
...
- **🤖 39 Built-in MCP Tools**: Comprehensive semantic tools over Streamable HTTP and stdio, providing agents safe search, structured reads, optimistic writes, and rollback controls. ([MCP Server Guide](docs/mcp_server.md))
...
- [Model Context Protocol (MCP) Reference](docs/mcp_server.md) — Canonical reference for all 39 tools, annotations, and schemas.

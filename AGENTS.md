...
The NexWiki MCP server registers and exposes **thirty-nine** semantic tools for AI agents, covering search and reads, article writes with optimistic locking, revision history and reverts, tag management, AI memory lifecycle, collaborative plans, the custom skills registry, headless evolution job lifecycle, progressive-disclosure orientation, backlink traversal, activity history, and OKF bundle import/export.
...
Every NexWiki `.md` file is a conformant **Open Knowledge Format (OKF v0.2)** concept document at rest (real YAML front matter). Each carries a `type` — exactly one of **`Wiki`** or the reserved **`AI-Agent-Memory`** / **`AI-Agent-Plan`** / **`AI-Agent-Skill`** classes, which only the agent tools set. The legacy `aiagent-*` *class* tags are gone; the class is now the `type`. System tags that remain: **status tags** (`get_status_tags`), tool-managed **memory-scope tags** (`memory-<scope>`), and the tool-managed **WikiSkill marker tags** (`trained-wikiskill`, `wikiskill-report`, `wikiskill-wiki`). A `wikiskill-wiki` article is wiki-scope — the accumulative wiki layer the evolution loop writes into: you may edit it freely, but `revert_article_version` and `delete_wiki_article` refuse it, nothing ever auto-deletes it, and you retire a replaced page with the `superseded_by` front-matter key (`edit_wiki_article`), which `wiki_health` reports as a chain.
...
Restart Claude Desktop, and you will see the **hammer icon 🔨** in the chat window, confirming that all thirty-nine NexWiki MCP tools are ready to use!
...
Cursor will establish a stream connection and immediately list all thirty-nine NexWiki tools in the sidebar. You can now use Cursor Composer or chat (`Cmd+K` / `Ctrl+K`) and reference your wiki directly during code generation!
...
5. **Respect Reserved Types**: Never relabel a reserved `AI-Agent-*` document `type` to a non-reserved one, and never strip a tool-managed `memory-<scope>` tag or a WikiSkill marker tag (`trained-wikiskill`, `wikiskill-report`, `wikiskill-wiki`). Call `get_status_tags` before applying lifecycle tags. A `wikiskill-wiki` article may be edited freely but never reverted or deleted — retire a replaced page with `superseded_by` instead.
